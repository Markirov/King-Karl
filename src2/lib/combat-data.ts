...
export function mechCalcHeatDelta(state: MechState, session: MechSession): HeatDelta {
  ...
  const weapons = state.weapons
    ...
    .reduce((sum, w) => sum + w.heat + (session.weaponMods?.[w.id]?.heat ?? 0), 0);
  const engineHeat = sysHits.engine * 5;

  // Calor extra de componentes modificados (gyro/reactor/etc.) — siempre activo, no depende de "uso"
  const critModsHeat = Object.values(session.critMods || {}).reduce((sum, m) => sum + (m.heat || 0), 0);

  const generated = move + weapons + engineHeat + critModsHeat;
